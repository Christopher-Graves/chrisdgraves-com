...
export default function ChatPage() {
  ...
  // Fetch gateway token from server
  useEffect(() => {
  ...
  const loadHistory = useCallback(async () => {
    try {
      const payload = await sendReq('chat.history', {});
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      send({
        ...
        id,
        method: 'chat.send',
        params: { message: text, idempotencyKey },
      } as GatewayMsg);
  ...
  const abortGeneration = () => {
    try {
      const id = nextId();
      send({ type: 'req', id, method: 'chat.abort', params: {} } as GatewayMsg);
    } catch {}
    setMessages((prev) => {
      const last = prev[prev.length - 1];
      if (last && last.streaming) {
